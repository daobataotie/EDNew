...
namespace Book.UI.Hr.Attendance.Atten
{
    internal partial class ReattenForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            //#region 更新基础薪资设置，还是设置自动吧，手动难免会有意外。
            //try
            //{
            //    BL.V.BeginTransaction();
            //    if (DateTime.Now.Day == 1)
            //    {
            //        //DateTime baseDate = Convert.ToDateTime(DateTime.Now.Year.ToString() + "/" + DateTime.Now.AddMonths(-1).Month.ToString() + "/1");
            //        DateTime baseDate = Convert.ToDateTime(DateTime.Now.AddMonths(-1).ToString("yyyy/MM") + "/1");
            //        IList<string> EmpIDList_1 = _clockdata.GetMakeSalaryList_DisEmpID(baseDate);
            //        BL.MonthlySalaryManager ms = new BL.MonthlySalaryManager();
            //        foreach (string EmpID in EmpIDList_1)
            //        {
