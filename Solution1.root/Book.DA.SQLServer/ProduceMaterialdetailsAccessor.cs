...
namespace Book.DA.SQLServer
{
    ...
    public partial class ProduceMaterialdetailsAccessor : EntityAccessor, IProduceMaterialdetailsAccessor
    {
        ...
        {
            string str = "select pmd.ProduceMaterialID,pm.ProduceMaterialDate,isnull(pmd.Materialprocessum,0) as Materialprocessum,isnull(pmd.Materialprocesedsum,0) as Materialprocesedsum,pmd.Distributioned,pmd.PronoteHeaderID,pmd.MRSHeaderId,pmd.ProductStock,xo.CustomerInvoiceXOId from ProduceMaterialdetails pmd left join ProduceMaterial pm on pmd.ProduceMaterialID=pm.ProduceMaterialID left join InvoiceXO xo on xo.InvoiceId=pm.InvoiceXOId where pm.ProduceMaterialDate BETWEEN '" + StartDate.ToString("yyyy-MM-dd") + "' AND '" + EndDate.AddDays(1).AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "' and pmd.productid='" + ProductId + "' union all select pomd.ProduceOtherMaterialId,pom.ProduceOtherMaterialDate,isnull(pomd.OtherMaterialQuantity,0),isnull(pomd.OtherMaterialALLUserQuantity,0),pomd.Distributioned,'',poc.MRSHeaderId,pomd.ProductStock,xo.CustomerInvoiceXOId from ProduceOtherMaterialDetail pomd left join ProduceOtherMaterial pom on pomd.ProduceOtherMaterialId=pom.ProduceOtherMaterialId left join ProduceOtherCompact poc on pom.ProduceOtherCompactId=poc.ProduceOtherCompactId left join InvoiceXO xo on xo.InvoiceId=poc.InvoiceXOId where pom.ProduceOtherMaterialDate BETWEEN '" + StartDate.ToString("yyyy-MM-dd") + "' AND '" + EndDate.AddDays(1).AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "' and pomd.ProductId='" + ProductId + "' order by ProduceMaterialID,ProduceMaterialDate";

            return this.DataReaderBind<Model.ProduceMaterialdetails>(str, null, CommandType.Text);
        }

        public double SelectMaterialprocessum(string id)
        {
            return sqlmapper.QueryForObject<double>("ProduceMaterialdetails.SelectMaterialprocessum", id);
        }
