...
namespace Book.DA.SQLServer
{
    ...
    public partial class StockAccessor : EntityAccessor, IStockAccessor
    {
        ...
        public void AnnualknotturnForm(string startdate, string enddate)
        {
            SqlParameter[] par = {                                  
            ...
            par[1].Value = enddate;
            SqlConnection conn = new SqlConnection(sqlmapper.DataSource.ConnectionString);
            SqlCommand cmd = new SqlCommand("DeleteCarryoverYear", conn);
            conn.Open();
        ...
        {
            return sqlmapper.QueryForObject<string>("Stock.GetLastDepotInDate", productid);
        }
    }
}